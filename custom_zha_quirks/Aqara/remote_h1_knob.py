...
try:
    ...
    from zhaquirks.xiaomi import XiaomiPowerConfiguration as PowerConfigurationClusterH1Remote
...
def _extract_report_attributes(args: Any) -> List[Any]:
    """Normalize zigpy report attributes payload across versions."""
    if args is None:
        return []

    if hasattr(args, "attribute_reports"):
        reports = args.attribute_reports
        return list(reports) if reports is not None else []

    if isinstance(args, (list, tuple)):
        if len(args) == 1 and hasattr(args[0], "attribute_reports"):
            reports = args[0].attribute_reports
            return list(reports) if reports is not None else []
        return list(args)
...
def _coerce_attr_value(raw_value: Any, attr_type: Any) -> Any:
    """Normalize zigpy attribute values across struct wrappers."""
    value = raw_value
...
class KnobManuSpecificCluster(XiaomiAqaraE1Cluster):
    ...
    def handle_cluster_general_request(
        ...
        *,
        ...
        event_args = {}
        for attr in _extract_report_attributes(args):
            if not hasattr(attr, "attrid") or not hasattr(attr, "value"):
                continue

            if attr.attrid in self.attributes:
                attr_def = self.attributes[attr.attrid]
                attr_name = getattr(attr_def, "name", str(attr.attrid))
                try:
                    attr_type = getattr(attr_def, "type", lambda x: x)
                    value = _coerce_attr_value(attr.value, attr_type)
                except Exception:
                    self.debug(
